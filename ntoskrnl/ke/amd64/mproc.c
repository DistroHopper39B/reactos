...
KeStartAllProcessors(VOID)
{
    ...
    while (TRUE)
    {
        ...
        PKGDTENTRY64 TssEntry = KiGetGdtEntry(&APInfo->Gdt, KGDT64_SYS_TSS);
        KiInitGdtEntry(TssEntry, (ULONG64)&APInfo->Tss, sizeof(KTSS64), AMD64_TSS, 0);

        /* Set up the TSS */
        APInfo->Tss.Rsp0 = (ULONG64)KernelStack;
        APInfo->Tss.Rsp1 = (ULONG64)KernelStack;
        APInfo->Tss.Rsp2 = (ULONG64)KernelStack;
        APInfo->Tss.Ist[0] = (ULONG64)DPCStack;
        APInfo->Tss.IoMapBase = 0xFFFF;

        /* Set up GDT */
        APInfo->Pcr.GdtBase = APInfo->Gdt; // shouldn't be needed
